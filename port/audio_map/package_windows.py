#!/usr/bin/env python3
"""Package Windows audio-map diagnostics without ROM or generated source."""
import argparse
import hashlib
import json
...
README = r"""JFG: inicialização do mapa de áudio — Windows x64

Exige Python x64 3.11+ e ROM US local, SHA-1
493ced9008dbe932d6e91179b68e8630cf23a023. A ROM não acompanha o pacote.

py -3 -I -O checks_map.py --library jfg_poc.dll --manifest map-config.json --rom 'C:\caminho\baserom.us.z64' --report map-report.json

Esperado: inicialização de áudio e mapa aprovados, com parada explícita antes de joyInit.
O mapa aloca e reinicia pools de sons. A thread de áudio apenas aguarda mensagens;
não processa frames ou amostras nem produz som ou imagem. Controles não são inicializados.
Não inicializa GPU nem modifica processos ou serviços preexistentes.

Documentação: https://github.com/thiagoribeiro269/Jet-Force-Gemini/tree/port/recomp-poc/port/audio_map
...
def main() -> None:
    ...
    parser.add_argument("--build", type=Path, default=ROOT / "build/port-audio-map")
    ...
    files.update({
        ...
        "map-config.json": (json.dumps(manifest, indent=2) + "\n").encode(),
    ...
    output = args.build / "jfg-audio-map-windows-x64.zip"
