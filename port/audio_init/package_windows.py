...
README = r"""JFG: preparação original dos bancos de áudio — Windows x64

Exige Python x64 3.11+ e ROM US local, SHA-1
493ced9008dbe932d6e91179b68e8630cf23a023. A ROM não acompanha o pacote.

py -3 -I -O checks_audio.py --library jfg_poc.dll --manifest audio-config.json --rom 'C:\caminho\baserom.us.z64' --report audio-report.json

Esperado: três cenários aprovados. Executa dados e heap de amInit, parando
antes de amCreateAudioMgr. Ainda não há saída de áudio, imagem ou boot completo.
Não inicializa GPU nem modifica processos ou serviços preexistentes.

Documentação: https://github.com/thiagoribeiro269/Jet-Force-Gemini/tree/port/recomp-poc/port/audio_init
...
def main():
    ...
    p.add_argument("--build", type=Path, default=ROOT / "build/port-audio-init")
    a = p.parse_args()
    manifest = json.loads((a.build / "proof/manifest.json").read_text())
    manifest.pop("rom_path", None); manifest.pop("elf_path", None)
    sources = {"checks_audio.py": "audio_init/checks_audio.py", "asset_checks.py": "audio_init/asset_checks.py", "checks_bootstrap.py": "bootstrap/checks_bootstrap.py",
    ...
    files.update({"jfg_poc.dll": (a.build / "windows/jfg_poc.dll").read_bytes(),
                  "audio-config.json": (json.dumps(manifest, indent=2) + "\n").encode(),
    ...
    output = a.build / "jfg-audio-init-windows-x64.zip"
