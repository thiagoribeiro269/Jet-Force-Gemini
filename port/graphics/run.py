#!/usr/bin/env python3
"""Build and compare one original model, texture, and captured display list."""
import argparse
...
def main() -> None:
    ...
    parser.add_argument("--out", type=Path, default=ROOT / "build/port-graphics")
    ...
    command("prepare", [sys.executable, ROOT / "port/graphics/prepare_graphics.py",
    ...
    command("graphics", [sys.executable, ROOT / "port/graphics/checks_graphics.py",
                         *common, "--report", out / "native-report.json"])
    command("reference", [sys.executable, ROOT / "port/graphics/verify_graphics.py",
                          *common, "--elf", args.elf.resolve(),
                          "--report", out / "mips-report.json"])
    print(f"Verified original model 35, texture 9097, and captured display list. Reports: {out}",
          flush=True)
