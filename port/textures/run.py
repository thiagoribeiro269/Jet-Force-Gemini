#!/usr/bin/env python3
"""Build and compare original texture and model initialization through objInitObjects."""
import argparse
...
def main() -> None:
    ...
    parser.add_argument("--out", type=Path, default=ROOT / "build/port-textures")
    ...
    command("prepare", [sys.executable, ROOT / "port/textures/prepare_textures.py",
    ...
    command("initialization", [sys.executable, ROOT / "port/textures/checks_textures.py",
                               *common, "--report", out / "native-report.json"])
    command("reference", [sys.executable, ROOT / "port/textures/verify_textures.py",
                          *common, "--elf", args.elf.resolve(),
                          "--report", out / "mips-report.json"])
    print(f"Verified texture and model initialization through objInitObjects. Reports: {out}", flush=True)
