...
def main():
    ...
    source = ROOT / "port/vendor/N64ModernRuntime/N64Recomp"
    if not (source / "lib/rabbitizer/include").is_dir():
    ...
    proof, toolchain, native = out / "proof", out / "toolchain", out / "native"
    ...
    command("build-tool", ["cmake", "--build", toolchain, "--config", "Release",
                           "--target", "N64RecompCLI", "--parallel", args.jobs], out)
    ...
    tool = find_artifact(toolchain, "N64Recomp.exe" if os.name == "nt" else "N64Recomp")
