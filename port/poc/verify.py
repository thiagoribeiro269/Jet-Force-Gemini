...
from unicorn import Uc, UcError, UC_ARCH_MIPS, UC_MODE_BIG_ENDIAN, UC_MODE_MIPS64
...
def original_overlay_link(oracle, manifest, section, base, rom):
    ...
    for pair in section["relocation_pairs"]:
        ...
        if result[2] != 2:
            raise AssertionError("Original linker did not consume a HI16/LO16 pair")
        ...
        expected = (base + pair["target_offset"]) & 0xFFFFFFFF
        if actual != expected:
...
def native_call(library, address, memory, registers):
    words = array.array("I")
    ...
    if status:
        raise RuntimeError(f"Native dispatcher refused {address:08X}: {status}")
    words.byteswap()
...
def run(manifest_path, library_path, report_path, cases_per_function):
    ...
    for function in manifest["functions"]:
        ...
        for base in bases:
            ...
            for case in range(cases_per_function):
                ...
                if section["overlay"]:
                    relocation_checks += original_overlay_link(oracle, manifest, section, base, rom)
    ...
    report = {
        ...
        "original_linker_pairs_executed": relocation_checks,
    ...
    print(f'PASS: {report["total_cases"]} differential cases; {relocation_checks} original-linker pairs.', flush=True)
    return report
