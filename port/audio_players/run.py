#!/usr/bin/env python3
"""Build and validate original audio-player creation through amGo."""
import argparse
...
def main():
    ...
    p.add_argument("--out", type=Path, default=ROOT / "build/port-audio-players")
    ...
    command("prepare", [sys.executable, ROOT / "port/audio_players/prepare_players.py", "--elf", a.elf.resolve(), "--rom", a.rom.resolve(), "--out", proof])
    ...
    command("initialization", [sys.executable, ROOT / "port/audio_players/checks_players.py", *common, "--report", out / "native-report.json"])
    command("reference", [sys.executable, ROOT / "port/audio_players/verify_players.py", *common, "--elf", a.elf.resolve(), "--report", out / "mips-report.json"])
    print(f"Verified audio-player creation through amGo. Reports: {out}", flush=True)
