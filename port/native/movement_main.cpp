...
int main(int argc, char **argv) {
    try {
        if (argc != 8)
            throw std::runtime_error("usage: jfg_native_movement CHARACTER SELECTOR REGION_MESH REGION_INFO COLLISION PHYSICS OUTPUT_PREFIX");
        ...
        const auto first = camera.view(*session.body(juno));
        ...
        auto source = [&](uint64_t tick) {
            ...
            camera.follow(*body, applied.stickX, applied.stickY);
            if (body->wall533) ++wallTicks;
            if (body->state568 == 3) ++airborneTicks;
            TickInput input;
            ActorInput actor; actor.control = applied = movementControl(tick, camera.yaw());
        ...
        for (unsigned frame = 0; frame < frames; ++frame) {
            ...
            const auto &pixels = renderer.draw(snapshot.renderInstances(), camera.view(body));
            ...
            trace << (frame ? "," : "") << "{\"frame\":" << frame << ",\"tick\":" << snapshot.hostTick << ",\"x\":" << body.position.x
                  ...
                  << ",\"clip\":" << snapshot.entities[0].clip << ",\"progress\":" << body.progress28 << ",\"camera_yaw\":" << camera.yaw()
        ...
        trace << ",\"stopped\":" << (stopped.entities.empty() && !stopped.assets && !stopped.region ? "true" : "false")
              << ",\"original_movement\":true,\"object_collision\":false,\"emulation\":false}\n";
