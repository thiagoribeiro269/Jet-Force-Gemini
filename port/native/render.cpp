...
static void render(const char *input, const char *prefix) {
    Reader reader(input);
    require(std::memcmp(reader.take(8), "JFGNAT1\0", 8) == 0, "Wrong native scene magic");
    const uint32_t textureCount = reader.u32(), drawCount = reader.u32(), vertexCount = reader.u32();
    require(reader.u32() == 0 && textureCount && textureCount <= 64 && drawCount && drawCount <= 1024 &&
    ...
    std::memcpy(vertices.data(), reader.take(vertices.size() * sizeof(Vertex)), vertices.size() * sizeof(Vertex));
    require(reader.offset == reader.bytes.size(), "Trailing scene payload");
    for (const auto &vertex : vertices) {
        const float *values = reinterpret_cast<const float *>(&vertex);
        for (unsigned i = 0; i < 9; ++i) require(std::isfinite(values[i]) && std::abs(values[i]) < 8192, "Invalid vertex value");
    }
    ...
    context->OMSetRenderTargets(1, colorView.GetAddressOf(), depthView.Get()); context->RSSetViewports(1, &viewport);
    ...
    context->Unmap(staging.Get(), 0); context->ClearState(); context->Flush();
    uint32_t colored = 0;
    ...
    report << "{\"status\":\"rendered_unreviewed\",\"api\":\"D3D11\",\"vendor\":" << adapterInfo.VendorId
           ...
           << ",\"emulator_dependencies\":false,\"display_list_interpreter\":false,\"animated\":false}\n";
    require(bool(report), "Cannot write report");
    std::cout << "Native D3D11 frame: " << vertexCount / 3 << " triangles, " << colored << " colored pixels\n";
}

int main(int argc, char **argv) {
    SetErrorMode(SEM_FAILCRITICALERRORS | SEM_NOGPFAULTERRORBOX);
    try { require(argc == 3, "usage: jfg_native_preview SCENE OUTPUT_PREFIX"); render(argv[1], argv[2]); return 0; }
