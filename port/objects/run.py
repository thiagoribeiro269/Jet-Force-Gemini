#!/usr/bin/env python3
"""Build and compare original object initialization through explosionFlushBlasts."""
import argparse
...
def main() -> None:
    ...
    parser.add_argument("--out", type=Path, default=ROOT / "build/port-objects")
    ...
    command("prepare", [sys.executable, ROOT / "port/objects/prepare_objects.py",
    ...
    command("initialization", [sys.executable, ROOT / "port/objects/checks_objects.py",
                               *common, "--report", out / "native-report.json"])
    command("reference", [sys.executable, ROOT / "port/objects/verify_objects.py",
                          *common, "--elf", args.elf.resolve(),
                          "--report", out / "mips-report.json"])
    print(f"Verified object initialization through explosionFlushBlasts. Reports: {out}", flush=True)
