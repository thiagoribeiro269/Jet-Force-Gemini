#!/usr/bin/env python3
"""Build and validate original audio manager and AI configuration."""
import argparse
...
def main():
    ...
    p.add_argument("--out", type=Path, default=ROOT / "build/port-audio-manager")
    ...
    proof, tool, native = out / "proof", ROOT / "build/port-bootstrap/jfg-toolchain", out / "native"
    ...
    command("prepare", [sys.executable, ROOT / "port/audio_manager/prepare_manager.py", "--elf", a.elf.resolve(), "--rom", a.rom.resolve(), "--out", proof])
    ...
    command("configure-native", ["cmake", "-S", ROOT / "port/poc", "-B", native, *gen, "-DCMAKE_BUILD_TYPE=Debug",
             "-DJFG_BOOT_PROFILE=ON", "-DJFG_THREAD_PROFILE=ON", "-DJFG_EVENT_PROFILE=ON", "-DJFG_INIT_PROFILE=ON", "-DJFG_AUDIO_MANAGER_PROFILE=ON",
    ...
    command("initialization", [sys.executable, ROOT / "port/audio_manager/checks_manager.py", *common, "--report", out / "native-report.json"])
    command("reference", [sys.executable, ROOT / "port/audio_manager/verify_manager.py", *common, "--elf", a.elf.resolve(), "--report", out / "mips-report.json"])
    command("frequency", [sys.executable, ROOT / "port/audio_manager/verify_frequency.py", *common, "--elf", a.elf.resolve(), "--report", out / "frequency-report.json"])
    print(f"Verified audio manager initialization. Reports: {out}", flush=True)
