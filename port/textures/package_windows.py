#!/usr/bin/env python3
"""Package Windows texture diagnostics without ROM or generated source."""
import argparse
import hashlib
import json
...
README = r"""JFG: inicialização de texturas — Windows x64

Exige Python x64 3.11+ e ROM US local, SHA-1
493ced9008dbe932d6e91179b68e8630cf23a023. A ROM não acompanha o pacote.

py -3 -I -O checks_textures.py --library jfg_poc.dll --manifest textures-config.json --rom 'C:\caminho\baserom.us.z64' --report textures-report.json
py -3 -I -O checks_protocol.py --library jfg_poc.dll --manifest textures-config.json --rom 'C:\caminho\baserom.us.z64' --report protocol-report.json

Esperado: mapa de áudio, joyInit, texInitTextures e modInitModels aprovados, com parada explícita antes de objInitObjects.
...
Documentação: https://github.com/thiagoribeiro269/Jet-Force-Gemini/tree/port/recomp-poc/port/textures
...
def main() -> None:
    ...
    parser.add_argument("--build", type=Path, default=ROOT / "build/port-textures")
    ...
    files.update({
        ...
        "textures-config.json": (json.dumps(manifest, indent=2) + "\n").encode(),
    ...
    output = args.build / "jfg-textures-windows-x64.zip"
