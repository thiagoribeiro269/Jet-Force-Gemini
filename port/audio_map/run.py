#!/usr/bin/env python3
"""Build and compare original audio-map initialization through joyInit."""
import argparse
...
def main() -> None:
    ...
    parser.add_argument("--out", type=Path, default=ROOT / "build/port-audio-map")
    ...
    command("prepare", [sys.executable, ROOT / "port/audio_map/prepare_map.py",
    ...
    command("initialization", [sys.executable, ROOT / "port/audio_map/checks_map.py",
                               *common, "--report", out / "native-report.json"])
    command("reference", [sys.executable, ROOT / "port/audio_map/verify_map.py",
                          *common, "--elf", args.elf.resolve(),
                          "--report", out / "mips-report.json"])
    print(f"Verified audio-map initialization through joyInit. Reports: {out}", flush=True)
