#!/usr/bin/env python3
"""Package Windows controller diagnostics without ROM or generated source."""
import argparse
import hashlib
import json
...
README = r"""JFG: inicialização de controles — Windows x64

Exige Python x64 3.11+ e ROM US local, SHA-1
493ced9008dbe932d6e91179b68e8630cf23a023. A ROM não acompanha o pacote.

py -3 -I -O checks_controllers.py --library jfg_poc.dll --manifest controllers-config.json --rom 'C:\caminho\baserom.us.z64' --report controllers-report.json
py -3 -I -O checks_protocol.py --library jfg_poc.dll --manifest controllers-config.json --rom 'C:\caminho\baserom.us.z64' --report protocol-report.json

Esperado: inicialização do mapa de áudio e joyInit aprovados, com parada explícita antes de texInitTextures.
A presença de controles é simulada por imports locais; nenhum controle físico é lido.
A thread de áudio apenas aguarda mensagens; não processa frames ou amostras nem produz som ou imagem.
Não inicializa GPU nem modifica processos ou serviços preexistentes.

Documentação: https://github.com/thiagoribeiro269/Jet-Force-Gemini/tree/port/recomp-poc/port/controllers
...
def main() -> None:
    ...
    parser.add_argument("--build", type=Path, default=ROOT / "build/port-controllers")
    ...
    files.update({
        ...
        "controllers-config.json": (json.dumps(manifest, indent=2) + "\n").encode(),
    ...
    output = args.build / "jfg-controllers-windows-x64.zip"
