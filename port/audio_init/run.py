#!/usr/bin/env python3
"""Build and validate original audio bank and sequence preparation."""
import argparse
...
def main():
    ...
    p.add_argument("--out", type=Path, default=ROOT / "build/port-audio-init")
    ...
    command("prepare", [sys.executable, ROOT / "port/audio_init/prepare_audio.py", "--elf", a.elf.resolve(), "--rom", a.rom.resolve(), "--out", proof])
    ...
    command("initialization", [sys.executable, ROOT / "port/audio_init/checks_audio.py", *common, "--report", out / "native-report.json"])
    command("reference", [sys.executable, ROOT / "port/audio_init/verify_audio.py", *common, "--elf", a.elf.resolve(), "--report", out / "mips-report.json"])
    print(f"Verified audio bank preparation. Reports: {out}", flush=True)
