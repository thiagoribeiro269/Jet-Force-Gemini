...
- Diretriz confirmada por Thiago em 25/09/2026: eliminar também o RT64 e toda emulação. Não copiar código de emuladores, simular CIC/PIF/RSP/RDP nem executar Unicorn como referência. A recompilação estática do código de CPU e a leitura dos formatos de assets continuam compatíveis com o port nativo. Implementar renderização por APIs nativas do Windows, com recursos e estruturas do PC. As provas antigas permanecem somente como histórico; não retomar seus runners de emulação. Os arquivos externos do CIC baixados na proposta rejeitada foram removidos antes de integração, compilação ou commit.
- Direção definida por Thiago em 25/09/2026: priorizar o entendimento global e a integração do port. Mapear fluxo completo, subsistemas e dependências; estruturar ciclo de aplicação, cenas, entidades e contratos de plataforma antes de continuar refinamentos isolados. As provas específicas servem como regressões e entram na arquitetura geral. Isso não revoga as restrições de emulação, acesso remoto ou trabalho solo.
