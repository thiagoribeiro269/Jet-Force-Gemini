#!/usr/bin/env python3
"""Build and validate original dynamic bootstrap and its next audio boundary."""
import argparse
...
def main():
    ...
    p.add_argument("--out", type=Path, default=ROOT / "build/port-bootstrap")
    ...
    command("prepare", [sys.executable, ROOT / "port/bootstrap/prepare_bootstrap.py", "--elf", a.elf.resolve(), "--rom", a.rom.resolve(), "--out", proof])
    ...
    command("initialization", [sys.executable, ROOT / "port/bootstrap/checks_bootstrap.py", *common, "--report", out / "native-report.json"])
    command("linker-safety", [sys.executable, ROOT / "port/bootstrap/checks_linker.py", *common, "--report", out / "linker-report.json"])
    command("reference", [sys.executable, ROOT / "port/bootstrap/verify_bootstrap.py", *common, "--elf", a.elf.resolve(), "--report", out / "mips-report.json"])
    print(f"Verified dynamic overlay bootstrap. Reports: {out}", flush=True)
