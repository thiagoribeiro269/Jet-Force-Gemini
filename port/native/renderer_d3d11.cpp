...
static ComPtr<ID3DBlob> compile(const char *entry, const char *target, CameraPreset camera) {
    ...
    const D3D_SHADER_MACRO macros[] = {{"JFG_WIDE_CAMERA", camera == CameraPreset::Wide ? "1" : "0"},
                                     {"JFG_CHARACTER_CAMERA", camera == CameraPreset::Character ? "1" : "0"}, {"JFG_INTEGRATION_CAMERA", camera == CameraPreset::Integration ? "1" : "0"}, {nullptr, nullptr}};
...
struct NativeRenderer::State {
    ...
    State(std::shared_ptr<const AssetPackage> package, CameraPreset camera) : assets(std::move(package)) {
        require(bool(assets) && !assets->vertices.empty(), "Empty renderer resources");
        SetErrorMode(SEM_FAILCRITICALERRORS | SEM_NOGPFAULTERRORBOX);
        const auto &vertices = assets->vertices;
        ...
        textureViews.resize(assets->textures.size());
        for (size_t i = 0; i < assets->textures.size(); ++i) {
            const auto &texture = assets->textures[i];
            ...
            check(device->CreateShaderResourceView(resource.Get(), nullptr, &textureViews[i]), "Texture view");
        }
        ...
        check(device->CreateSamplerState(&sampler, &samplerState), "Native sampler");
    ...
    const std::vector<uint8_t> &draw(const std::vector<RenderInstance> &instances) {
        ...
        for (const auto &instance : instances) {
            require(instance.mesh == 0 && instance.bones.size() == assets->skeleton.size(), "Invalid render resource or skeleton");
        ...
        for (unsigned alpha = 0; alpha < 2; ++alpha) for (const auto &vertices : posed) {
            D3D11_MAPPED_SUBRESOURCE mapped{};
            check(context->Map(buffer.Get(), 0, D3D11_MAP_WRITE_DISCARD, 0, &mapped), "Update native mesh");
            std::memcpy(mapped.pData, vertices.data(), vertices.size() * sizeof(Vertex));
            context->Unmap(buffer.Get(), 0);
            for (const auto &draw : assets->draws) {
                ...
                context->PSSetShaderResources(0, 1, textureViews[draw.texture].GetAddressOf());
...
NativeRenderer::NativeRenderer(std::shared_ptr<const AssetPackage> assets, CameraPreset camera)
    : state_(std::make_unique<State>(std::move(assets), camera)) {}
NativeRenderer::~NativeRenderer() = default;
const std::vector<uint8_t> &NativeRenderer::draw(const std::vector<RenderInstance> &instances) { return state_->draw(instances); }
