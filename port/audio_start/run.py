#!/usr/bin/env python3
"""Build and validate audio thread startup through the amInitAudioMap boundary."""
import argparse
...
def main():
    ...
    parser.add_argument("--out", type=Path, default=ROOT / "build/port-audio-start")
    ...
    command("prepare", [sys.executable, ROOT / "port/audio_start/prepare_start.py",
    ...
    command("build-native", ["cmake", "--build", native, "--config", "Debug",
                             "--parallel", args.jobs])
    command("smoke", ["ctest", "--test-dir", native, "-C", "Debug",
                      "--output-on-failure"])
    ...
    command("initialization", [sys.executable, ROOT / "port/audio_start/checks_start.py",
                               *common, "--report", out / "native-report.json"])
    command("wake", [sys.executable, ROOT / "port/audio_start/checks_wake.py",
                     *common, "--report", out / "wake-report.json"])
    command("reference", [sys.executable, ROOT / "port/audio_start/verify_start.py",
                          *common, "--elf", args.elf.resolve(),
                          "--report", out / "mips-report.json"])
    print(f"Verified audio thread startup through amInitAudioMap. Reports: {out}", flush=True)
